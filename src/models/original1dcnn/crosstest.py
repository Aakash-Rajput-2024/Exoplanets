"""Cross-generator evaluation for the original 1D-CNN model.

  python src/models/original1dcnn/crosstest.py --engine taurex
...
def main():
    ...
    model = NasaInaraModel(in_channels=1, sequence_length=4379)
    run_crosstest(
        model=model,
        checkpoint_path=os.path.join(HERE, "checkpoints", "model_best.pth"),
        cache_dir=cache_dir,
        inara_stats_dir=os.path.join(REPO, "data", "cache_original"),
        out_dir=HERE,
        model_name=f"NasaInaraModel (1-ch planet) [eval={args.engine}]",
    )
