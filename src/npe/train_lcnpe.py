...
def main():
    ...
    if not os.path.exists(aug_x_full_path):
        ...
        x_train = torch.load(aug_x_full_path, map_location="cpu")
        y_train = torch.load(aug_y_full_path, map_location="cpu")
    ...
    density_estimator = inference.train(training_batch_size=64, learning_rate=5e-4, show_info=True)
