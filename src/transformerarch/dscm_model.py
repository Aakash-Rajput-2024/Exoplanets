...
the v2 pipeline:
  ...
  * A byte-for-byte DIVERGED copy of these same classes also existed inside
    src/causal/cnn_trnas/model.py -- identical class names / state_dict shapes,
    ...
    too (src/causal/cnn_trnas/model.py now defines only PositionalEncoding and
    NasaInaraTransformer).

Only src/transformerarch/train_dscm.py (itself legacy/unused by the v2
