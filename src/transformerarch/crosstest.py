"""Cross-generator evaluation for the transformer model.

Mirrors test.py but evaluates the trained checkpoint on synthetic spectra from a
different physics generator (TauREx / petitRADTRANS), using INARA training stats.

  python src/transformerarch/crosstest.py --engine taurex
...
def main():
    ...
    run_crosstest(
        ...
        out_dir=HERE,
        model_name=f"NasaInaraTransformer (1-ch planet) [eval={args.engine}]",
    )
