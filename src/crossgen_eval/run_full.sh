...
$VENV "$REPO/src/crossgen_eval/build_eval_cache.py" --engine taurex \
...
$TORCH "$REPO/src/transformerarch/crosstest.py" --engine taurex 2>&1 | grep -E "HEADLINE|Avg|wrote.*details"
echo "=== [4/6] original1dcnn crosstest ==="
$TORCH "$REPO/src/original1dcnn/crosstest.py" --engine taurex 2>&1 | grep -E "HEADLINE|Avg|wrote.*details"
echo "=== [5/6] causal crosstest ==="
$TORCH "$REPO/src/causal/cnn_trnas/crosstest.py" --engine taurex 2>&1 | grep -E "HEADLINE|Avg|wrote.*details"
