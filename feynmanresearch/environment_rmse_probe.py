#!/usr/bin/env python
"""No-source-edit environment RMSE probe.

Goal: estimate causal generalization by grouping validation examples by nuisance
environments from data/summary.csv and reporting worst-environment RMSE.

Caveat: this reconstructs the cached random validation split using the current
os.listdir order and seed used by src/transformerarch/dataloader.py. If the cache
was created under a different file order, exact membership may differ.
"""
import csv
import json
import os
...
OUT_DIR = REPO / "feynmanresearch" / "environment_probe"
...
def main():
    max_samples = int(os.environ.get("MAX_ENV_SAMPLES", "1024"))
    ...
    mean_x = torch.load(CACHE_DIR / "mean_x.pt", map_location="cpu")
    std_x = torch.load(CACHE_DIR / "std_x.pt", map_location="cpu")
    ...
    # Shape from first cached/raw example.
    ...
    result = {
        ...
        "caveat": "Reconstructed validation split from current file order and seed; use a saved index split for publication-grade results.",
        "rows": rows,
    }
